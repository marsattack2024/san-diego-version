...
**Status:** Step 8 (Testing) IN PROGRESS. Automated tests created/updated.
...
8.  **(IN PROGRESS) Testing:**
    ...
    *   **A. (DONE) Unit Tests for `ChatSetupService` (`tests/unit/chat-engine/chat-setup.service.test.ts`):** Verified core config logic.
    *   **B. (DONE) Integration Tests for API Routes (`tests/integration/api/chat-route.test.ts`, `tests/integration/api/widget-chat-route.test.ts`):** Verified route handler logic and service interactions (with mocks).
    *   **C. (DONE) Update Existing Unit Tests:** Reviewed relevant unit tests (`deep-search.test.ts`); confirmed alignment with changes.
    *   **D. (TODO) Manual Verification (Smoke Testing):** Needs to be performed by running the application.
