...
We have standardized the API routes that require authentication:

✅ **Implemented**:
- Updated `app/api/chat/route.ts` to use `withAuth`
- Updated `app/api/chat/[id]/route.ts` (GET, PATCH) to use `withAuth`
- Updated `app/api/chat/[id]/messages/route.ts` to use `withAuth`
- Updated `app/api/chat/[id]/messages/count/route.ts` to use `withAuth`
- Updated `app/api/chat/session/route.ts` (POST, GET) to use `withAuth`
- Updated `app/api/history/route.ts` (GET, DELETE) to use `withAuth`
- Updated `app/api/history/invalidate/route.ts` to use `withAuth`
- Updated `app/api/profile/notification/route.ts` to use `withAuth`
- Updated `app/api/vote/route.ts` to use `withAuth`
- Manually implemented auth check in `app/api/profile/update-summary/route.ts` due to Node.js runtime incompatibility with the current `withAuth` wrapper.

**Implementation Notes**:
- Applied the `withAuth` wrapper to all relevant routes, ensuring consistent authentication handling.
- Removed manual `supabase.auth.getUser()` calls from protected routes.
- Verified that routes correctly receive the authenticated `user` object.
- Confirmed RLS policies are the primary mechanism for data authorization after authentication.
- The `update-summary` route required manual auth due to its Node.js runtime.

**Testing Strategy**:
- Tested API endpoints with authenticated and unauthenticated requests.
- Verified correct status codes (200, 401, 500) are returned.
- Confirmed error formats are consistent.
...
1. **Final Cleanup**
   - Remove deprecated files and components (`circuit-breaker-debug.tsx`, old `utils/supabase/middleware.ts`)
