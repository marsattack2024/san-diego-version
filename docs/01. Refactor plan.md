...
- `lib/chat/validator.ts` (3.6KB, 114 lines) - Replaced by direct validation in route handler
...
- ❌ `lib/chat/validator.ts` - Replaced by direct validation in route handler
- ❌ `lib/vector/rag-cache.ts` - Consolidated into lib/cache/redis-client.ts for unified Redis handling
- ❌ `lib/chat-engine/tools/rag-tool.ts` - Consolidated into knowledge-base.ts to avoid duplicate functionality
...
- ✅ `lib/vector/documentRetrieval.ts` → `lib/services/vector/document-retrieval.ts` - Moved to services folder with kebab-case naming
- ✅ `lib/vector/embeddings.ts` → `lib/services/vector/embeddings.ts` - Moved to services folder to follow service pattern
- ✅ `lib/vector/types.ts` → `lib/services/vector/types.ts` - Moved to services folder to follow service pattern