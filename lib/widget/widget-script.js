...
(function() {
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      console.log('Sending message to widget API:', text);
      ...
      // Improved error handling for non-200 responses
      if (!response.ok) {
      ...
      try {
        ...
        if (!responseText.trim()) {
          ...
          message.textContent = "I'm sorry, I couldn't find specific information about that in my knowledge base. Please try asking in a different way or about another topic.";
        }
        
        console.log('Final response text:', responseText);
      } catch (streamError) {
        console.error('Error processing stream:', streamError);
        message.textContent = "Sorry, there was an error processing the response stream. Please try again.";
      }
    } catch (error) {
