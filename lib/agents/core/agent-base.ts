...
export abstract class BaseAgent implements Agent {
  ...
  async processMessage(
    ...
    logger.info(`Processing message with ${this.name}`, {
    ...
    try {
      ...
      const aiTools = this.tools.reduce((acc, t) => {
        acc[t.name] = tool({
          ...
          execute: async (params) => {
            logger.debug(`Executing tool ${t.name}`, {
              tool: t.name,
              params
            });
            ...
            try {
              ...
              logger.debug(`Tool ${t.name} executed successfully`, {
                tool: t.name,
              ...
              logger.error(`Error executing tool ${t.name}`, {
      ...
      logger.info(`Completed processing message with ${this.name}`, {
      ...
      logger.error(`Error processing message with ${this.name}`, {
        error: error instanceof Error ? error.message : String(error),
